...
isoProjInterpolate <- function(init_fn, grids){
  ...
  # redundent_index <- iso_fn_evals[which(diff(iso_fn_evals) > -0.0001)]
  print("pava done")
  iso_fn <- approxfun(grids[non_redundent_index], iso_fn_evals[non_redundent_index], method = "linear",rule=2) # linear interpolation
  ...
  # iso_fn_deriv <- approxfun((grids[-K] + grids[-1]) / 2, diff(iso_fn_evals)/diff(grids),
...
isotonizeDensity <- function(densities, grids, k=1000){
  ...
  res = isoProjInterpolate(init_scores, (Q(u[-1]) + Q(u[-(k+1)])) / 2)
  psi = res[["iso_fn"]]
...
############# Method A: kde based #############
## truncation_lim <- c(alpha, gamma), p'<alpha, p>gamma
# kde_decr_score_est <- function(residuals, k=1000, kernel="gaussian", kernel_pts=2^23,
#                                truncation_lim=NULL){
#   require(fdrtool)
#   require(Iso)
#   require(stats)
...
#   # Interpolate between density values on the grid
#   fs <- kde$y
#   fs <- (fs[-length(fs)] + fs[-1]) / 2
#   fprimes <- diff(kde$y) / diff(kde$x)
#   init_scores <- fprimes / fs
#   ####truncation
#   if(!is.null(truncation_lim)){
#     init_scores[(fs < gamma) | (fprimes > alpha)] <- 0
#   }
#   xs <- kde$x
#   xs <- (xs[-length(xs)] + xs[-1]) / 2
#   init_score <- approxfun(xs, init_scores, method = "linear", rule = 2)
#   Fx <- cumsum(kde$y * c(0, diff(kde$x)))
#   Fx <- Fx / Fx[length(kde$x)]
#   # In case F has constant pieces:
#   Fx[duplicated(Fx)] <- Fx[duplicated(Fx)] + 1e-12
#   Q <- approxfun(Fx, kde$x, method = "linear", rule = 2, ties = max)
#   k <- max(k, 2 * length(residuals))
#   u <- 0:k / k
# 
#   init_scores <- init_score(Q(u)) ##psihat\circle Qhat
#   # cat("The largest abs score = ", max(abs(init_scores)), '\n')
#   
#   res = isoProjInterpolate(init_scores, Q(u))
#   
#   return(list("psi"=res[["iso_fn"]], "psi_deriv"=res[["iso_fn_deriv"]]))
# }


kde_decr_score_est <- function(residuals, k=1000, kernel="gaussian", kernel_pts=2^20,
  ...
  res = isoProjInterpolate(init_scores, (Q(u[-1]) + Q(u[-length(u)]) / 2))
  print("isoproj done")
  return(list("psi"=res[["iso_fn"]], "psi_deriv"=res[["iso_fn_deriv"]]))
  # return(list("psis"=res[["iso_fn"]], "iso_fn_x" = res[["iso_fn_x"]],
  #             "psi_derivs"=res[["iso_fn_deriv"]], "iso_fn_deriv_x" = res[["iso_fn_deriv_x"]]))
}
...
antitonicScoreMatch <- function(Z, lambda0){
    require(MASS)
    N <- length(Z)
    lambda <- N * lambda0
...
############# need to be fixed ###############
spline_linear_regression_newton <- function(betainit, X, Y, knots, psivals, psiprimes, n_iter = 16,
